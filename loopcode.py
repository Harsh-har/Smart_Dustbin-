...
def capture_frame(cam_url):
    ...
    logging.info(f"Camera shot '{cam_url}' done successfully")
    return frame
...
def process_first_layer(cam_1):
    ...
    for _ in range(4):  # Run process 4 times
        frame = capture_frame(cam_1)
        if frame is None:
            return  # Stop processing if no object detected

        polythene_detected = detect_polythene("temp.jpg")
        print('First camera processed the object.')

        if polythene_detected:
            print("Polythene detected! Activating cutter and opening plate.")
            send_command('H')
            time.sleep(1)
            send_command('C')
            time.sleep(1)
...
def detect_biodegradable(frame):
    """Run YOLO detection multiple times to classify garbage."""
    detected_flags = []
    
    for _ in range(5):  # Run detection 5 times for accuracy
        ...
        if detected_flags:
            ...
            logging.info(f"Flag '{selected_flag}' sent.")
            return selected_flag  # Stop looping if detection is successful
        else:
...
def process_second_third_layer(cam_2):
    ...
    for _ in range(4):  # Run process 4 times
        print("\n--- Starting Second & Third Layer Processing ---")
        for _ in range(5):  # Run up to 5 attempts
            ...
            if frame is None:
                print("❌ No object detected. Stopping process.")
                return  # Stop if no object is detected

            print("🧪 Running Object Detection")
            detected_flag = detect_biodegradable(frame)
            if detected_flag is None:
                print("⚠️ No valid classification. Stopping process.")
                return  # Stop if no valid classification is found

            print(f"🔁 Rotating Third Layer Hand Motor to position {detected_flag}")
            send_command(f"HAND_MOTOR:{detected_flag}")
            time.sleep(2)
...
if __name__ == "__main__":
    for i in range(4):  # Run the entire sorting process 4 times
        print(f"\n🚀 Starting Sorting Cycle {i+1}/4")
